...
def getdata_Grain_casein(fs,c):
    ...
    dataName = 'soil.mat'
    dataName_select='soil_select.mat'
    ...
    if fs == 'pca':
        ...
        lis = data_select['soil_cars'][0][c][0]
        lis = lis
        n_components = len(lis)
        print(n_components)
        datax = datax[:, lis]
    elif fs == 'iriv':
        lis = data_select['soil_iriv'][0][c][0]
        lis = lis - 1
        n_components = len(lis)
        print(n_components)
        datax = datax[:, lis]
    elif fs == 'vcpa':
        lis = data_select['soil_vcpa'][0][c][0]
        lis = lis - 1
        n_components = len(lis)
        print(n_components)
        datax = datax[:, lis]
    ...
    datax_train=datax[0:81,:]
    datax_test=datax[81:108,:]
    datay_train = datay[0:81]
    datay_test = datay[81:108]
    print(np.max(datay_train), np.min(datay_train), np.mean(datay_train), np.std(datay_train), np.std(datay_train))
    print(np.max(datay_test), np.min(datay_test), np.mean(datay_test), np.std(datay_test), np.std(datay_test))
    # datax = np.append(datax_train, datax_test, axis=0)
    # datay = np.append(datay_train, datay_test, axis=0)
...
if __name__ == '__main__':
    getdata_Grain_casein(fs='iiv',c=1)
