...
def getdata_Grain_casein(fs,c):
    ...
    # print(np.max(datay_train),np.min(datay_train),np.mean(datay_train),np.std(datay_train), np.std(datay_train,ddof=1))
    # print(np.max(datay_test), np.min(datay_test), np.mean(datay_test), np.std(datay_test), np.std(datay_test,ddof=1))
