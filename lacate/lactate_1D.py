...
def grain_casein_1D(aa, cc, i, v, f, s):
    ...
    callback_list_test = \
        [
            ModelCheckpoint(filepath='F://NIRATSpecNet-main//grain//grain_casein//h5_1D//' + now_s + '.h5',
                            # 文件路径 存在当前路径下吧 还好找
    ...
    model = load_model('F://NIRATSpecNet-main//grain//grain_casein//h5_1D//' + now_s + '.h5', )
...
if __name__ == '__main__':
    for bb in range(1, 3):  # 变量选择方法
        for cc in range(3, 20):  # 20个已选变量
            for t in range(0, 1):  # 激活函数：t
                for s in range(0, 1):  # 权值初始化 s
                    for q in range(0, 5):  # 优化器 q
                        for w in (2,):  # 卷积核大小 w
