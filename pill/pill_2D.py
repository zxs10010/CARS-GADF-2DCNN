...
def grain_casein_2D(aa,cc,i, v, f, s):
    ...
    callback_list_test = \
        [
            ModelCheckpoint(filepath='F://NIRATSpecNet-main//grain//grain_casein//h5_2D//' + now_s + '.h5',
                            # 文件路径 存在当前路径下吧 还好找
    ...
    # data_gaf = np.load('data_gaf.npz')
    # train_data_mtf = data_gaf['train_data_mtf']
    # test_data_mtf = data_gaf['test_data_mtf']
    # print(train_data_mtf.shape)
    # print(test_data_mtf.shape)


    # diesel_spec, diesel_prop = sF.getdata_chaiyou()
    # test_data, test_lable, train_data, train_lable = sF.getTestData_chaiyou(diesel_spec, diesel_prop)
    size = train_data.shape[1]
    '''print(test_data.shape)
    print(test_data)
    test_data = test_data.reshape(20, 1, 700)
    print(test_data.shape)
    print(test_data)
    # print(test_data[0][0])
    ...
    # 求train_data训练数据的马尔可夫变迁场 mtf
    # print(train_data.shape)   # train_data.shape=(60,700,1)
    # print(train_data)
    train_data = train_data.reshape(a, 1, n_components)
    # print(train_data.shape)  # train_data.shape=(60,1,700)
    # print(train_data)
    train_data0 = train_data[0]
    # print(train_data0.shape)
    # mtf = MarkovTransitionField(image_size=image_size, n_bins=n_bins)
    # mtf=GramianAngularField(image_size=image_size)
    mtf = GramianAngularField(image_size=n_components, method='difference')
    # mtf=GramianAngularField(image_size=image_size)
    train_data_mtf = mtf.fit_transform(train_data0)
    # print(train_data_mtf.shape)  # train_data_mtf.shape=(1,700,700)
    # print(train_data_mtf)
    for i in range(1, a):
        train_data_ = train_data[i]
        train_data_mtf_ = mtf.transform(train_data_)
        train_data_mtf = np.append(train_data_mtf, train_data_mtf_, axis=0)
    # train_data_mtf_1 = np.append(train_data_mtf, train_data_mtf, axis=0)
    # train_data_mtf = np.append(train_data_mtf_1, train_data_mtf, axis=0)
    # train_data_mtf_3 = np.append(train_data_mtf_2, train_data_mtf, axis=0)
    # train_data_mtf = np.append(train_data_mtf_3, train_data_mtf, axis=0)

    # print(train_data_mtf)
    print(train_data_mtf.shape)

    '''train_data_mtf = train_data_mtf[0]
    # print(train_data_mtf.shape)  # train_data_mtf.shape=(700,700)
    print(train_data_mtf)
    # Show the image for the first time series
    plt.figure(figsize=(5, 5))
    plt.imshow(train_data_mtf, cmap='rainbow', origin='lower')
    plt.title('Markov Transition Field', fontsize=18)
    plt.colorbar(fraction=0.0457, pad=0.04)
    plt.tight_layout()
    plt.show()'''

    # 求test_data测试数据的马尔可夫变迁场 mtf
    test_data = test_data.reshape(b, 1, n_components)
    test_data0 = test_data[0]
    test_data_mtf = mtf.transform(test_data0)
    # print(test_data_mtf.shape)  # test_data_mtf.shape=(1,700,700)
    # print(test_data_mtf)
    for e in range(1, b):
        test_data_ = test_data[e]
        test_data_mtf_ = mtf.transform(test_data_)
        test_data_mtf = np.append(test_data_mtf, test_data_mtf_, axis=0)
    # test_data_mtf_1 = np.append(test_data_mtf, test_data_mtf, axis=0)
    # test_data_mtf = np.append(test_data_mtf_1, test_data_mtf, axis=0)
    # test_data_mtf_3 = np.append(test_data_mtf_2, test_data_mtf, axis=0)
    # test_data_mtf = np.append(test_data_mtf_3, test_data_mtf, axis=0)
    print(test_data_mtf.shape)
    # np.savez('data_gaf', test_data_mtf=test_data_mtf, train_data_mtf=train_data_mtf)



    # input = layers.Input(shape=(n_components, n_components, 1), dtype='float32')
    # net = layers.Conv2D(16, 3, activation=jihuo, padding='same',
    #                     kernel_initializer=kernel_initializer, )(input)
    #
    # b_1 = layers.Conv2D(16, 7, activation=jihuo, padding='same',
    #                     kernel_initializer=kernel_initializer, )(net)
    # b_1 = layers.Conv2D(16, 3, activation=jihuo, padding='same',
    #                     kernel_initializer=kernel_initializer, )(b_1)
    # b_1 = layers.Conv2D(8, 3, activation=jihuo, padding='same',
    #                     kernel_initializer=kernel_initializer, )(b_1)
    #
    # b_2 = layers.Conv2D(16, 5, activation=jihuo, padding='same',
    #                     kernel_initializer=kernel_initializer, )(net)
    # b_2 = layers.Conv2D(16, 3, activation=jihuo, padding='same',
    #                     kernel_initializer=kernel_initializer, )(b_2)
    # b_2 = layers.Conv2D(8, 3, activation=jihuo, padding='same',
    #                     kernel_initializer=kernel_initializer, )(b_2)
    #
    ...
    # history = model.fit(train_data_mtf, train_lable,
    #                     epochs=epochs_au,
    #                     batch_size=batch_size_au,
    #                     validation_data=(test_data_mtf, test_lable),
    #                     callbacks=callback_list_test
    #                     )

    # sF.drawLoss(history)  # 绘制当前的验证曲线
    # now_s='2024-04-09-06-00-08'
    now_s = '2023-11-02-06-24-39'
    model = load_model('F://NIRATSpecNet-main//grain//grain_casein//h5_2D//' + now_s + '.h5', )
    # model = load_model('model.h5', custom_objects={'GroupNormalization': GroupNormalization()})
    result_trian = model.predict(train_data_mtf)
    result_predict = model.predict(test_data_mtf)
    ...
    # 下面的代码用于自动记录实验数据
    write_data=[(test_lable,result_predict)]
    # write_data = [
    ...
    write_to_csv_2D_diff(write_data,select_method,cc)
    return rmsep, r_2_p


if __name__ == '__main__':
    for bb in range(0, 1):  # 变量选择方法
        for cc in range(11, 12):  # 20个已选变量
            for t in range(0, 1):  # 激活函数：t
                for s in range(0, 1):  # 权值初始化 s
                    for q in range(0, 1):  # 优化器 q
                        for w in (2,):  # 卷积核大小 w
                            grain_casein_2D(bb,cc,q, t, w, s)
